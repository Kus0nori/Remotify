...
2. **Устройство** (`Views/DeviceDetailView.swift`) — три варианта экрана:
   - телефон не в сети ПК (`LocalNetworkMonitor` сравнивает IP ПК с подсетями
     `en*`/`bridge*` телефона) — «Подключитесь к локальной сети»;
   - в сети, но ПК не отвечает — «ПК не отвечает» и кнопка Wake-on-LAN (пока неактивна);
   - ПК на связи — основной экран ниже.

   Точка статуса связи справа от
...
- **Wake-on-LAN и broadcast.** С iOS 14 отправка на broadcast-адреса (включая
  `255.255.255.255` и `x.x.x.255`) требует `com.apple.developer.networking.multicast`,
  который выдаётся по заявке и только платным аккаунтам. Без него остаётся unicast
  на IP ПК: будит из сна (ARP offload), но не после выключения. В симуляторе
  broadcast работает — проверять только на устройстве.
